...
const { scanCodeNotes, buildDependencyGraph, buildSetupGuide, deriveProjectCapabilities, buildArchitectureFlow, rankCoreFiles } = require('./codeIntel');
 ...
 */
// Extensions analyseSourceFile() can extract function/class summaries from
const ANALYSABLE_EXTENSIONS = new Set(['.js', '.ts', '.mjs', '.cjs', '.jsx', '.tsx', '.py']);

const ADD_PATTERN    = /^Added (class|function) `([^`]+)`$/;
const REMOVE_PATTERN = /^Removed (class|function) `([^`]+)`$/;

/**
 * Turns raw signals + freshly-extracted docstring summaries into a short
 * human title and an "impact" sentence describing the new capability.
 * e.g. signals=["Added class `ConfidenceValidator`"], summaries has its
 * docstring -> { title: "Added `ConfidenceValidator`", impact: "Validates..." }
 * Returns { title: null, impact: null } if nothing useful is found —
 * callers fall back to the existing line-count `summary` string.
 */
function synthesizeChangeNarrative(signals, summaries) {
  summaries = summaries || {};
  let best = null;

  for (const sig of signals || []) {
    const m = ADD_PATTERN.exec(sig);
    if (!m) continue;
    const [, kind, name] = m;

    let qualified = name;
    let doc = summaries[name];
    if (!doc) {
      // Might be a class method, stored as "ClassName.methodName"
      const key = Object.keys(summaries).find((k) => k.endsWith(`.${name}`));
      if (key) { qualified = key; doc = summaries[key]; }
    }
    if (doc) {
      const candidate = { title: `Added \`${qualified}\``, impact: doc };
      if (kind === 'class') return candidate; // class additions are most significant
  ...
  for (const sig of signals || []) {
    const m = REMOVE_PATTERN.exec(sig);
    if (m) return { title: `Removed \`${m[2]}\``, impact: null };
  }
...
function buildCodeChangeEntry(relPath, oldContent, newContent, action, timestamp) {
  ...
  // Treat requirements*.txt as "code" too, so dependency-add/remove
  // signals get extracted (extractCodeSignals dispatches on this).
  const isCode = CODE_EXTENSIONS.has(ext) || /requirements.*\.txt$/i.test(relPath);
  ...
  if (action === 'add' && !oldContent) {
    ...
    let title = null, impact = null;
    if (ANALYSABLE_EXTENSIONS.has(ext)) {
      const analysis = analyseSourceFile(newContent || '', ext);
      ({ title, impact } = synthesizeChangeNarrative(signals, analysis.summaries));
    }
  ...
  let title = null, impact = null;
  if (ANALYSABLE_EXTENSIONS.has(ext)) {
    const analysis = analyseSourceFile(new_, ext);
    ({ title, impact } = synthesizeChangeNarrative(signals, analysis.summaries));
  }
...
function createDefaultState(projectRoot) {
  ...
  const state = {
    ...
    // Per-directory subsystem breakdown: every documented class/method
    // grouped by its top-level directory (full detail, not just primary)
    module_capabilities: derived.byDir
  };
  ...
  state.core_files        = rankCoreFiles(codeFiles, depGraph, state.setup_guide);
  return state;
}
...
function genAiSummary(state, boot) {
  ...
  // If the project has documented subsystems (v2.2.0+ derived from class
  // docstrings + dependency graph), describe it by those — far more
  // informative than a generic "<Language> application." for projects
  // (e.g. Python) where the JS-dependency-based signals find nothing.
  const dirs = Object.keys(state.module_capabilities || {});
  if (dirs.length >= 2) {
    return `${type}${db} organized into ${dirs.length} subsystems: ${dirs.map((d) => `${d}/`).join(', ')}.`;
  }
...
async function updateProjectState(projectRoot, changeEvent, options) {
  ...
  const nextState = {
    ...
    // Per-directory subsystem breakdown (full detail, not just primary)
    module_capabilities: freshDerived.byDir
  };
