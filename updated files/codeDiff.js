...
function extractCodeSignals(patch, filePath) {
  ...
  } else if (ext === 'txt' && /requirements/i.test(filePath || '')) {
    extractRequirementsSignals(patch, signals);
  } else if (ext === 'md') {
    signals.push('Documentation updated');
  }

  // Cross-language: flag newly added TODO/FIXME/HACK/XXX/BUG notes
  const addedLines = patch.split('\n').filter((l) => l.startsWith('+')).map((l) => l.slice(1));
  extractNoteSignals(addedLines, signals);

  return uniqueSignals(signals);
}

/**
 * Detects newly-added TODO / FIXME / HACK / XXX / BUG comments
 * (in // # or <!-- --> style) and surfaces them as signals so the
 * changelog and briefing show "this change introduced a known gap".
 */
function extractNoteSignals(addedLines, signals) {
  const notePattern = /(?:\/\/|#|<!--)\s*(TODO|FIXME|HACK|XXX|BUG)\b[:\s-]*(.*?)(?:-->)?\s*$/i;
  let count = 0;
  for (const line of addedLines) {
    if (count >= 3) break;
    const m = notePattern.exec(line);
    if (m) {
      const tag  = m[1].toUpperCase();
      const text = (m[2] || '').trim().slice(0, 80);
      signals.push(`Added ${tag}${text ? ': ' + text : ''}`);
      count++;
    }
  }
}
...
/**
 * Detects Python package additions/removals in requirements.txt-style
 * files. Handles lines like "pandas==2.1.0", "loguru>=0.7", "ccxt".
 */
function extractRequirementsSignals(patch, signals) {
  const addedLines   = patch.split('\n').filter((l) => l.startsWith('+')).map((l) => l.slice(1));
  const removedLines = patch.split('\n').filter((l) => l.startsWith('-')).map((l) => l.slice(1));
  const pkgPattern   = /^\s*([A-Za-z0-9_.-]+)\s*(?:[=<>~!]{1,2}.*)?$/;

  for (const line of addedLines) {
    const t = line.trim();
    if (!t || t.startsWith('#')) continue;
    const m = pkgPattern.exec(t);
    if (m) signals.push(`Added package \`${m[1]}\``);
  }
  for (const line of removedLines) {
    const t = line.trim();
    if (!t || t.startsWith('#')) continue;
