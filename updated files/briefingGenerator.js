...
function buildRecentChanges(state) {
  ...
  for (const c of changes.slice(0, 10)) {
    ...
    if (c.title) {
      lines.push(`**${c.title}**${c.impact ? ` — ${c.impact}` : ''}`);
      lines.push(`_${c.summary}_`);
    } else {
...
function buildInstructions() {
  return [
    ...
    '6. Read **Project Evolution** to understand what capabilities were added and when.',
    '7. Read **Recent Code Changes** for the exact diffs of what just changed.',
    '8. Read **Architecture Flow** to understand how data moves through the system.',
    '9. Read **Module Capabilities** for a per-subsystem breakdown of what each part does.',
    '10. Read **Code Catalogue** to know what each file/function/class does before calling anything.',
    '11. Read **Module Dependencies** to understand how files connect before moving code around.',
    '12. Read **API Routes** before adding or changing any endpoint.',
    '13. Read **Dependencies** before suggesting `npm install` — the package may already exist.',
    '14. Read **Working Context** to understand the current focus and locked-in decisions.',
    '15. Read **What To Do Next** for prioritised next actions.',
    '',
...
function buildCoreFiles(state) {
  const core = state.core_files || [];
  if (!core.length) return '';
  const lines = [sectionHeader('🚀 Start Here')];
  lines.push('_Read these files first — they are the most central to understanding this project._\n');
  core.forEach((c, i) => {
    lines.push(`**${i + 1}. \`${c.file}\`** — ${c.reason}`);
    if (c.description) lines.push(`   ${c.description}`);
  });
  return lines.join('\n');
}

function buildArchitectureFlowSection(state) {
  const flow = state.architecture_flow;
  if (!flow || !flow.length) return '';
  const lines = [sectionHeader('🌊 Architecture Flow')];
  lines.push('_How data flows through the system — from entry point to its deepest dependencies._\n');
  lines.push('```');
  lines.push(...flow);
  lines.push('```');
  return lines.join('\n');
...
function buildModuleCapabilities(state) {
  const caps = state.module_capabilities || {};
  const dirs = Object.keys(caps);
  if (!dirs.length) return '';
  const lines = [sectionHeader('🧩 Module Capabilities')];
  lines.push('_Per-subsystem breakdown — what each directory does and which components it contains._\n');
  for (const dir of dirs) {
    const classes = caps[dir] || [];
    if (!classes.length) continue;
    lines.push(`#### \`${dir}/\``);
    for (const cls of classes) {
      lines.push(`- **\`${cls.name}\`** — ${cls.summary}`);
    }
    lines.push('');
  }
  return lines.join('\n');
...
function buildProjectEvolution(state) {
  const changes = state.code_changes || [];
  if (!changes.length) return '';

  // Group by date, keep only entries with a title
  const byDate = {};
  ...
  const lines = [sectionHeader('📅 Project Evolution')];
  lines.push('_Semantic history of meaningful changes — what capability was added or changed and when._\n');
  for (const date of dates) {
    lines.push(`**${date}**`);
    for (const c of byDate[date].slice(0, 4)) {
      lines.push(`- ${c.title}${c.impact ? ` — ${c.impact}` : ''}`);
    }
    lines.push('');
  }
...
function generateBriefing(state, projectRoot) {
  ...
  const sections = [
    ...
    buildProjectEvolution(state),
    buildRecentChanges(state),
    buildArchitectureFlowSection(state),
    buildModuleCapabilities(state),
